...
import com.lxp.model.courses.Content;
...
public class ContentRepository {
    ...
    public Content getContentById(Long contentId) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    return new Content(rs.getLong("content_id"), rs.getLong("section_id"),
                            rs.getString("content_title"), rs.getString("content_url"),
                            rs.getInt("time"), rs.getBoolean("is_public"),
    ...
    public Long insertContent(ContentInsertDTO contentInsert) {
        String sql = QueryUtils.getQuery("create.content");

        try (PreparedStatement pstmt = connection.prepareStatement(sql,
                Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setLong(1, contentInsert.getSectionId());
            pstmt.setString(2, contentInsert.getContentTitle());
            pstmt.setString(3, contentInsert.getContentUrl());
            pstmt.setInt(4, contentInsert.getTime());
