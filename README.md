...
LXP 사이트를 직접 구현해봄으로써, `Java` 및 `JDBC` 기반의 `CRUD`를 통해 `Controller`, `Service`, `Repository` 영역의 역할에 대해 깊이 이해하고, 이를
...
1. 강좌 개설 및 조회, 수정, 삭제가 가능하다. (강좌 삭제의 경우 `soft delete`가 적용된다)
